...
public:
	...
	static void   SetLocalAddress(u_long theAddr) { IPAddr::SetLocalHost((long)theAddr); }
	static u_long GetLocalAddress(void) { return (u_long)IPAddr::GetLocalHost(); }
...
*/
	...
	void NotifyBlockedSendTime(time_t theTime) {  if(mFirstBlockedSendTime==0) mFirstBlockedSendTime = theTime; }
	void NotifySuccessfulSend() {  mFirstBlockedSendTime = 0; }

	WONStatus SetNagling(bool on);
	WONStatus SetAllowBroadcast(bool allowBroadcast);

	WONStatus GetLastError() { return mLastError; }
...
// Only should be used by SocketThreads
	...
	RefCountPtr GetThreadDataSafe() { return mThreadData; }

	CriticalSection& GetCrit() { return mDataCrit; }
