...
public:
	EGPrivateKey();
	explicit EGPrivateKey(unsigned short theLen);
	EGPrivateKey(unsigned short theLen, const unsigned char* theKeyP);
	EGPrivateKey(const EGPrivateKey& theKey);
	...
	void Create(unsigned short theLen, const unsigned char* theKeyP);
	void Create(unsigned short theLen=0);
...
private:
	mutable WONCryptMini::ElGamal mElGamal;   // Decrypt object
	mutable EGPublicKey mPubKey;
