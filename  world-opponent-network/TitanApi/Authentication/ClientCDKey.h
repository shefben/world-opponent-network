...
//#include "../common/won.h"	// WON 3/21/00
#include <string>
...
// RawBuffer type (string of bytes)
// Also defined in won.h, but don't want extra header dependencies, so it is
// redefined here.
namespace WONCommon {
...
public:
	...
	explicit ClientCDKey(const std::string& theProductR, bool storeInHKeyCurUser = false);	// WON 3/21/00
...
protected:
	...
	bool				  mStoreInHKeyCurUser;   // Store key only for user (as opposed to entire machine)	// WON 3/21/00
...
private:
	...
	// Class methods
	static void RemoveSkipChars(std::string& theStrR);	// WON 3/21/00
