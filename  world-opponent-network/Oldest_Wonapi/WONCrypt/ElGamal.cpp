...
bool ElGamal::encrypt(const BigInteger &M, BigInteger ab[]) const
{
	try
	{
		BigInteger p_minus_1 = p.sub(BigInteger::BigInt_ONE);
		...
		} while (!(k.gcd(p_minus_1).equals(BigInteger::BigInt_ONE)));
...
bool ElGamal::BogusSign(const BigInteger &M, BigInteger ab[]) const
{
	try 
	{
		...
		do
		{
			...
			//s = (k - x*r) % q;
		} while (ab[0].cmp(BigInteger::BigInt_ZERO)==0);			// make sure r != 0
                      ...
                      BigInteger p, BigInteger g, BigInteger y) 
{
    BigInteger p_minus_1 = p.subtract(ONE);
    // sanity checks
    if (M.compareTo(BigInt_ZERO) < 0 || M.compareTo(p_minus_1) >= 0 ||
        a.compareTo(BigInt_ZERO) < 0 || a.compareTo(p_minus_1) >= 0 ||
        b.compareTo(BigInt_ZERO) < 0 || b.compareTo(p_minus_1) >= 0)
        return false;
...
// check a != 0 && a == (g^b * y^a + m) mod q
	...
	try
	{
		if(a.cmp(BigInteger::BigInt_ZERO)==0)
			return false;
