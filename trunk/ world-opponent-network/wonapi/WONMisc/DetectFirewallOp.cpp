...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void DetectFirewallOp::CloseSocketNow(bool kill)
{
	if(mListenSocket.get()!=NULL)
	{
		mListenSocket->Close();
...
WONStatus DetectFirewallOp::GetNextRequest()
{
	...
	if (!mUseUDP)
	{
		aBuf.AppendBool(mWaitForConnect); // wait for connect
...
WONStatus DetectFirewallOp::CheckResponse()
{
	...
	if(aMsg.Available()>=4)
	{
		unsigned long aHost = aMsg.ReadLong();
		if(aHost!=0)
			mPublicAddr.Set(aHost,0);
	}
