...
#ifdef _MSC_VER
#include <string>
#ifndef __WON_VC_STL_HEADER_PATCH__
#pragma message(" ")
#pragma message("WARNING: You may need to copy the header files from the 'VC STL Header Patches' directory into the VC include directory.")
#pragma message("The headers which ship with VC 4-6 contain bugs which these patches address.")
#pragma message(" ")
#endif
#endif
...
void WONAPICore::QueueInContainer(AsyncOp *theOp)
{
	...
	{
		// Don't allow new ops to run but let the final ops still running finish
		if(!theOp->Pending())
			mInstance->mOpContainer.QueueOp(theOp);
	}
