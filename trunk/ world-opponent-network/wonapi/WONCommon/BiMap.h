...
#ifdef _CPPLIB_VER
// Begun Dinkumware SXL patch
	struct KeyItrStruct
	{
		KeyItr mItr;
		KeyItrStruct(const KeyItr &theItr) : mItr(theItr) { }
		KeyItr& operator=(const KeyItr &theItr) { mItr = theItr; return mItr;}
		operator KeyItr&() { return mItr; }
	};

	typedef std::multimap<Val, KeyItrStruct, ValMapPred> ValMap;
// End Dinkumware SXL patch
#else
// Begin More efficient and proper way
	typedef std::multimap<Val, KeyMap::iterator, ValMapPred> ValMap;
// End More efficient and proper way
#endif
