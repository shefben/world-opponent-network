...
static bool CopyClient(SmartRoutingConnection::ClientInfo &theClientCopy, RoutingClientInfo *theClient)
{
	...
	theClientCopy.mCDKeyId = aSmartClient->mCDKeyId;
	return true;
}
...
{
	RoutingClientMap::iterator anItr = mClientMap.insert(RoutingClientMap::value_type(theClientId, NULL)).first;
...
void SmartRoutingConnection::SetClientCDKeyIdFromDataObject(unsigned short theClientID, ByteBufferPtr theBuf)
{
	...
	theSmartClientInfo->mCDKeyId = anId;
}
...
{
	RoutingClientInfoPtr aClient = theOp->GetClientInfo(); //GetNewClientInfo();
//	aClient->mFlags = theOp->GetClientFlags();
...
bool SmartRoutingConnection::HandleCommand(const std::wstring &theInput, int theGroupIdContext)
{
	...
	unsigned long aNonClientCDKeyIdToBan = -1;
	...
	if(needName)
	{
		...
		// Is this a clientID or user name?
		...
		if (mLastInputCommand == InputCommand_ServerBan && (*mLastMatchName.begin() == '%'))
		{
			mLastMatchName.erase(mLastMatchName.begin());
			aNonClientCDKeyIdToBan = atoi(WStringToString(mLastMatchName).c_str());
		}
		else
		{		
	...
	switch(mLastInputCommand)
	{
		...
		{		
			...
			if (mLastInputCommand == InputCommand_ServerMute || mLastInputCommand == InputCommand_ServerBan)
				allowPerm = true;

			DWORD aSeconds = 3600; // 1 hour
			string aTimeStr = WStringToString(GetWord(theInput,aPos,true,false));
			if(aTimeStr=="INFINITE" && allowPerm)
				aSeconds = 0;
			...
			// Get comment
			wstring aComment = GetWord(theInput,aPos,false,true);
			switch(mLastInputCommand)
			{
				case InputCommand_Mute:
					MuteClientByClientId(theGroupIdContext,true,aSeconds,aComment,mLastTargetClient->mId); 
					break;
			
				case InputCommand_ServerMute:
					MuteClientByClientId(RoutingId_Global,true,aSeconds,aComment,mLastTargetClient->mId); 
					break;

				case InputCommand_Ban:
					BanClientByClientId(theGroupIdContext,true,aSeconds,aComment,mLastTargetClient->mId); 
					break;

				case InputCommand_ServerBan:
					if (aNonClientCDKeyIdToBan == -1)
						BanClientByClientId(RoutingId_Global,true,aSeconds,aComment,mLastTargetClient->mId);
					else
						BanClientByCDKeyId(RoutingId_Global,true,aSeconds,aComment,aNonClientCDKeyIdToBan);
					break;
			}
...
void SmartRoutingConnection::AddDefaultUserCommandStrings()
{
	...
	AddCommandString(L"WHISPER",	InputCommand_Whisper);
