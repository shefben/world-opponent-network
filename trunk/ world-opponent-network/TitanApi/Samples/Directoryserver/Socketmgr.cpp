...
Error 
SocketMgr::SendSMsgToServer(BaseMessage& theMsgR)
{
	Error aReturn;
...
SocketMgr::ReestablishPersistentTCP()
{
	...
	SMsgDirG2StatusReply* aReplyP = NULL;
	if ( (mLastError = ((IPSocket*)mTMsgSocketP->GetSocket())->Open(&mAddress, -1, false) ) == Error_Success)
	{
		aMsg.Pack();
		...
		aReplyP = (SMsgDirG2StatusReply*)mTMsgSocketP->RecvBaseMsg((WONMsg::BaseMessage**)NULL, 8000, false); // Wait 2 seconds
		aSucceeded = true;
	}
	
	if (aReplyP != NULL && aSucceeded)
	{
		SMsgDirG2StatusReply aReply = *aReplyP;
