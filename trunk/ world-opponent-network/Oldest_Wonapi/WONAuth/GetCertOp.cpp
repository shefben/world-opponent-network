...
void GetCertOp::RunHook()
{
	...
	const AuthPeerData* aPeerData = mAuthContext->GetPeerData();
...
WONStatus GetCertOp::CheckResponse()
{
	...
	if(mState==GETTING_PUB_KEYS)
	{
		...
		mKeyBlock = new AuthPubKeyBlock(aMsg.ReadBytes(aLen),aLen);
		if(!mKeyBlock->IsValidAndVerified(mAuthContext->GetVerifierKey()))
		{
			SetLastServerError(WS_GetCert_InvalidPubKeyBlock);
			return WS_ServerReq_TryNextServer;
		}
