...
struct DirACL
{
	...
	DirACL(char theType, unsigned long theUser, unsigned long theCommunity, unsigned long theTrust) :
		mType(theType), mUser(theUser), mCommunity(theCommunity), mTrust((unsigned short)theTrust) {}
};
...
enum DirGetFlags
{
	...
	DIR_GF_DIRADDORIGIP    = 0x00080000,  // Add originating IP address (admins only)
	...
	DIR_GF_SERVADDORIGIP   = 0x08000000   // Add originating IP address (admins only)
};
...
enum DirUpdateFlags
{
	...
	DIR_UF_DIRNOACLINHERIT   = 0x10,	// Do notinherit parent ACLs for AddDirs
};
