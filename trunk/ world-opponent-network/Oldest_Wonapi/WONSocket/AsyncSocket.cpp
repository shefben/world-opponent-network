...
WONStatus AsyncSocket::Bind(unsigned short thePort, bool allowReuse) 
{
	...
	if ( mType == UDP || mType == TCP ) 
	{
		...
		memcpy( &aSockAddr, &aSockAddrIn, sizeof(SOCKADDR) ); // sizeof(SOCKADDR) == sizeof(SOCKADDR_IN)
	}

	aVal = ::bind(mSocket,(SOCKADDR*)&aSockAddr,sizeof(SOCKADDR_IN));
	if(aVal!=0)
		return TSGetLastError();
